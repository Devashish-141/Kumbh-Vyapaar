...
export function RoleCard({
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.98 }}
      onClick={onClick}
      className={`relative overflow-hidden rounded-2xl p-8 text-left transition-all w-full group ${
        isVisitor
    ...
    >
      ...
      <div
        className={`absolute -right-8 -top-8 w-32 h-32 rounded-full opacity-10 transition-opacity group-hover:opacity-20 ${
          isVisitor ? "bg-primary" : "bg-teal-light"
        }`}
      />
      ...
      <p
        className={`text-sm font-medium mb-1 ${
          isVisitor ? "text-primary" : "text-teal-light"
        }`}
      >
        {subtitle}
      </p>
      ...
      <p
        className={`text-sm leading-relaxed ${
          isVisitor ? "text-muted-foreground" : "text-secondary-foreground/80"
        }`}
      >
        {description}
      </p>
